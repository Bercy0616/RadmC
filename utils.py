...
def get_mean_std(excel_path):
    train = pd.read_csv(excel_path)
    mean = []
    std = []
    print('calculating mean and std..')
    pbar = tqdm(total = train.shape[0])
    for i in range(train.shape[0]):
        path = train.loc[i,'linux_pathv3']
        ...
        pbar.update(1)
    pbar.close()
    print('mean: %.2f, std: %.2f'%(np.mean(mean),np.mean(std)))
    return [np.mean(mean),np.mean(std)]

if __name__ == '__main__':
    get_mean_std(r'table/train.csv')